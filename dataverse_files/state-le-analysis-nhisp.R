 ...
 # 2/21/2013 - fixed to use non hispanic data
 # ignoring arriaga decomp to speed things up a little. just putting zeros in those cells of output
 ##############################################################################################
 
 #Functions
 #################################################################
 # a couple of functions for timing the runs
...
#functions to convert risks to rates and vice versa
#risktorate<-function(x,t) {
#    -log(1-x)/t
#}
...
# load data
# load("C:/bugsR/deathstotal.dta")
load("C:/bugsR/deathtotalnhisp.RData")

colnames(dtotal)<-c("state","age","gender","race","year","crude","deaths","pop")
dtotal$crude=as.numeric(dtotal$crude)
dtotal$deaths=as.numeric(dtotal$deaths)
dtotal<-dtotal[dtotal$age!=19,]
...
#for (istate in c(35:40,42:44,46:50)) {
for (istate in c(32)){
for (igender in 0:1) {
...
#age group
agew=testw$age
#log of population for use in poisson model
popw=rep(0,dim(testw)[1])
#p-yrs for a given year: if death's are observed, 1yr*number living year + .5*number dying
popw[is.na(testw$deaths)==FALSE]=log(testw$pop[is.na(testw$deaths)==FALSE]-0.5*testw$deaths[is.na(testw$deaths)==FALSE])
#p-yrs for a given year: if death's aren't observed, #living-5
#but if pop size<9, then give pop size/2
#popw[is.na(testw$deaths)==TRUE]=log(testw$pop[is.na(testw$deaths)==TRUE]-5)
popw[is.na(testw$deaths)==TRUE&testw$pop>=10]=log(testw$pop[is.na(testw$deaths)==TRUE&testw$pop>=10]-5)
popw[is.na(testw$deaths)==TRUE&testw$pop<10]=log(testw$pop[is.na(testw$deaths)==TRUE&testw$pop<10]/2)

#pop in binomial model
#popw=testw$pop
...
testb=testb[testb$year>=4,]
testb<-testb[order(testb$deaths),]
N2b=length(testb$year)
yb=testb$deaths
popb=rep(0,dim(testw)[1])
popb[is.na(testb$deaths)==FALSE]=log(testb$pop[is.na(testb$deaths)==FALSE]-0.5*testb$deaths[is.na(testb$deaths)==FALSE])
popb[is.na(testb$deaths)==TRUE&testb$pop>=10]=log(testb$pop[is.na(testb$deaths)==TRUE&testb$pop>=10]-5)
popb[is.na(testb$deaths)==TRUE&testb$pop<10]=log(testb$pop[is.na(testb$deaths)==TRUE&testb$pop<10]/2)
...
#run openbugs
modelCheck("c:/bugsR/poisopenltny.bug")
...
r.b=array(0,dim=c(19,16,samplesSize("b[1,1]")))
namesb=samplesMonitors("b")
#age/year specific rates for whites
r.w=array(0,dim=c(19,16,samplesSize("w[1,1]")))
namesw=samplesMonitors("w")
#exponentiate MCMC iterates to get rates
for (i in 1:19) {
    for (j in 1:16){
        r.b[i,j,]=exp(samplesSample(namesb[(i-1)*16 + j]))
        r.w[i,j,]=exp(samplesSample(namesw[(i-1)*16 + j]))       
        }
    }
...
#        temp[c1,]<-c(i,j,quantile(tot[i,j,],c(.025,.5,.975),na.rm=TRUE),quantile(de[i,j,],c(.025,.5,.975),na.rm=TRUE),quantile(ie[i,j,],c(.025,.5,.975),na.rm=TRUE))        
        temp[c1,]<-c(i,j,
                    ...
                    mean(lew[1,16,]-lew[1,1,]),quantile(lew[1,16,]-lew[1,1,],c(.025,.975)),
                    mean(leb[1,16,]-leb[1,1,]),quantile(leb[1,16,]-leb[1,1,],c(.025,.975)),
                    mean(lediff[1,16,]-lediff[1,1,]),quantile(lediff[1,16,]-lediff[1,1,],c(.025,.975))                    
                    )        
        c1<-c1+1
    }
}
...
# the resulting file is called "state-le-results-nhisp.dta"
