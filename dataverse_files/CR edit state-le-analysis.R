...
# load data
#load("C:\\Users\\MacLehose\\Documents\\projects\\srj\\test.rdata")
...
#CR commented out: dtotal$crude <- as.numeric(dtotal$crude)
#CR commented out: dtotal$deaths <- as.numeric(dtotal$deaths)
...
for (istate in c(35:40, 42:44, 46:50)) { #CR question: why running the for loop for istate over different ranges separately?
        ...
        popw <- rep(0, 380)
        popw[is.na(testw$deaths) == FALSE] <- log(testw$pop[is.na(testw$deaths) == FALSE] - 
                                                   0.5*testw$deaths[is.na(testw$deaths) == FALSE])
        ...
        yb <- testb$deaths
        popb <- rep(0, 380)
        popb[is.na(testb$deaths) == FALSE] <- log(testb$pop[is.na(testb$deaths) == FALSE] - 
                                                   0.5*testb$deaths[is.na(testb$deaths) == FALSE])
        ...
        #run openbugs
        modelCheck("c:/bugsR/poisopenlt.bug")
