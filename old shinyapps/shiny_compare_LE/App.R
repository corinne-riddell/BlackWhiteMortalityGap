...
ui1 <- fluidPage(#theme = shinytheme("cosmo"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      plotlyOutput("sum_missing_white"))
  )
...
server <- function(input, output) {
  ...
  output$sum_missing_white <- renderPlotly({
    temp.dat <- (dat.clean[dat.clean$State2 == input$state & dat.clean$Race2 == "White" & dat.clean$Sex2 == "Male", c("Age2", "Population", "Year", "Count", "COD2")])
