...
```{r figure5, fig.align="center", fig.height=8, fig.width=8, echo=F}
ggplotly(ggplot(subset(BlackWhite, Sex2 == "Male"), aes(y=WBgap.s, x = Year3)) + geom_line(aes(col = State2))
         + facet_wrap(~Census_Region) +
           ...
           xlab("Year") +
           geom_text(data = subset(BlackWhite, Year3 == 2013 & Sex2 == "Male"), aes(label = stabbrs), check_overlap = T) #+
...
```{r figure6, fig.align="center", fig.height=8, fig.width=8, echo=F}
ggplotly(ggplot(subset(BlackWhite, Sex2 == "Female"), aes(y=WBgap.s, x = Year3)) + geom_line(aes(col = State2))
         + facet_wrap(~Census_Region) +
           ylab("Black-white life expectancy gap (years)") +
           xlab("Year") +
           geom_text(data = subset(BlackWhite, Year3 == 2013 & Sex2 == "Female"), aes(label = stabbrs), check_overlap = T))
```

**Figures 7 and 8 to be added: Trends in the contribution of CVD to the black-white life expectancy gap (in years) by US state**
-make one for males and one for females
-separately for each COD group
-included CIs
