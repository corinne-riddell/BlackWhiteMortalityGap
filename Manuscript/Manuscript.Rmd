...
The data produced two statistical issues that were addressed by the statistical methods we chose. First, our methodology needed to impute the missing death counts. Second, several states had small blacks populations which produced even smaller population sizes in each sex- and age-specific grouping. In these cases, the estimation of the age-specific mortality rates is imprecise, and produces abrupt changes in the estimation of mortality rates over time that are unrealistic. To better estimate the underlying age-specific mortality rates, a method that produces smoother age-specific mortality trends was required.

KTM to add bayesian modeling paragraph

- type of bayesian model used and what was modeled Y vs X, separately for each state, race, and sex
- how the smoothing happens over year
- how the truncated poisson is involved
- this methodology produces smoothed death counts 
- sampled from posterior distribution with thinning
- software: R software packages and their versions

Thus, the model was run separately for every combination of state, race, and sex, and produced death counts that had been smoothed over time for every cause of death within each age group. To calculate life expectancy at birth, the smoothed death counts were aggregated to estimate the total deaths in each age group and standard life table methods were applied [ref]. Life expectancy at birth (in years) was calculated for blacks and whites in each state and year according to sex. The difference in life expectancies between blacks and white was calculated by subtracting the black life expectancy from the white life expectancy in every state, year, and sex.

We then performed a decomposition analysis using the Arriaga method, to decompose the black-white difference in life expectancy according to age group and cause of death [ref]. The decomposition analysis partitions the difference in life expectancy by age group, which can be interpreted as the amount of the gap (in years) that can be attributed to higher mortality rates for each age group. For example, a decomposition of a gap of five years may find that two of the five years (40% of the gap) is attributable to a higher mortality rate in blacks in the youngest age group (< 1 years old) compared with the youngest whites. The age-specific decomposition is then further partitioned according to cause of death, producing the number of years of the gap that is attributable to each age-cause-of-death combination. For example, a decomposition of a gap of 8 years may find that 1.3 years of this gap is attributable to more injury-related deaths in blacks aged 25-29 years old. We also calculated the marginal contribution of each cause of death by summing the cause-specific contributions across age groups.
...
```{r setup_figure1_and_2, echo=F}
year1 <- 1969
year2 <- 2013

bw_dat <- BlackWhite %>% 
  filter(Year3 %in% c(year1, year2), is.na(WBgap.s) == F) %>%
  select(State2, Sex2, Year3, WBgap.s) %>%
  arrange(Year3)

bw_dat <- tidyr::spread(bw_dat, Year3, WBgap.s) 
names(bw_dat)[3:4] <- c("first.gap","second.gap")  
...
cod.df <- data.frame(State2 = factor(), Sex2 = factor(), COD = factor(), Contribution.to.change = numeric(), Contrib.to.change.prop = numeric(), narrowed_gap = logical())
age.df <- data.frame(State2 = factor(), Sex2 = factor(), Ages = factor(), Contribution.to.change = numeric(), Contrib.to.change.prop = numeric(), narrowed_gap = logical())
    
for (state.i in levels(BlackWhite$State2)) {
  
  for (sex.i in c("Male", "Female")) {
  first.index = which(paired.ids$State2 ==  state.i & paired.ids$Year3 == year1 & paired.ids$Sex2 == sex.i)
  second.index = which(paired.ids$State2 ==  state.i & paired.ids$Year3 == year2 & paired.ids$Sex2 == sex.i)
  ...
  if(is.null(list.cod.marginal.tables.smoothed[[first.index]][1]) == FALSE & 
    ...
    temp2 <- data.frame(State2 = state.i, Sex2 = sex.i,
    ...
    temp3 <- data.frame(State2 = state.i, Sex2 = sex.i,
...
##### COD ####
cod.df.male <- make_dataset_cod_plot(cod.decomp.table = subset(cod.df, Sex2 == "Male"), age.groups = "State2", 
                                 cause.of.death = "COD", sign.var = "narrowed_gap", 
                                 decomp.var = "Contribution.to.change", decomp.var.prop = "Contrib.to.change.prop")

cod.df.female <- make_dataset_cod_plot(cod.decomp.table = subset(cod.df, Sex2 == "Female"), age.groups = "State2", 
                                 cause.of.death = "COD", sign.var = "narrowed_gap", 
                                 decomp.var = "Contribution.to.change", decomp.var.prop = "Contrib.to.change.prop")

cod.df <- merge(rbind(cod.df.male, cod.df.female), bw_dat, by = c("State2", "Sex2"))

cod.df <- cod.df %>% mutate(new.start = -start + first.gap, new.finish = -finish + first.gap)

#### Age ####
age.df.male <- make_dataset_cod_plot(cod.decomp.table = subset(age.df, Sex2 == "Male"), age.groups = "State2", 
                                  cause.of.death = "Ages", sign.var = "narrowed_gap", 
                                  decomp.var = "Contribution.to.change", decomp.var.prop = "Contrib.to.change.prop")
age.df.female <- make_dataset_cod_plot(cod.decomp.table = subset(age.df, Sex2 == "Female"), age.groups = "State2", 
                                  cause.of.death = "Ages", sign.var = "narrowed_gap", 
                                  decomp.var = "Contribution.to.change", decomp.var.prop = "Contrib.to.change.prop")

age.df <- merge(rbind(age.df.male, age.df.female), bw_dat, by = c("State2", "Sex2"))
...
ggplotly(
  ggplot(subset(cod.df, Sex2 == "Male"), aes(y = State.gdiff.num, x = new.start)) +
    geom_rect(aes(xmin = new.start, ymin = State.gdiff.num - 0.45, 
                  ymax = State.gdiff.num + 0.45, xmax = new.finish, fill = COD), color = "white") +
    geom_segment(data = subset(bw_dat, Sex2 == "Male"), aes(x = second.gap, xend= second.gap, y = State.gdiff.num - 0.5, yend = State.gdiff.num + 0.5)) +
    geom_segment(data = subset(bw_dat, Sex2 == "Male"), aes(x = first.gap, xend= first.gap, y = State.gdiff.num - 0.5, yend = State.gdiff.num + 0.5), lty = 3) +
...
ggplotly(
  ggplot(subset(cod.df, Sex2 == "Female"), aes(y = State.gdiff.num, x = new.start)) +
    geom_rect(aes(xmin = new.start, ymin = State.gdiff.num - 0.45, 
                  ymax = State.gdiff.num + 0.45, xmax = new.finish, fill = COD), color = "white") +
    geom_segment(data = subset(bw_dat, Sex2 == "Female"), aes(x = second.gap, xend= second.gap, y = State.gdiff.num - 0.5, yend = State.gdiff.num + 0.5)) +
    geom_segment(data = subset(bw_dat, Sex2 == "Female"), aes(x = first.gap, xend= first.gap, y = State.gdiff.num - 0.5, yend = State.gdiff.num + 0.5), lty = 3) +
    theme_minimal() + scale_y_continuous(breaks = 1:x.num.breaks, labels = x.labels)) %>% 
  layout(xaxis = list(title = "Life expectancy gap (years)"), yaxis = list(title = NA, autorange = "reversed"))
...
ggplotly(                                     
  ggplot(subset(age.df, Sex2 == "Male"), aes(y = State.gdiff.num, x = new.start)) +
...
ggplotly(                                     
  ggplot(subset(age.df, Sex2 == "Female"), aes(y = State.gdiff.num, x = new.start)) +
    geom_rect(aes(xmin = new.start, ymin = State.gdiff.num - 0.45, ymax = State.gdiff.num + 0.45, xmax = new.finish, fill = Ages), color = "white") + 
    geom_segment(data = subset(bw_dat, Sex2 == "Female"), aes(x = second.gap, xend= second.gap, y = State.gdiff.num - 0.5, yend = State.gdiff.num + 0.5)) + 
    geom_segment(data = subset(bw_dat, Sex2 == "Female"), aes(x = first.gap, xend= first.gap, y = State.gdiff.num - 0.5, yend = State.gdiff.num + 0.5), lty = 3) + 
    theme_minimal() + scale_y_continuous(breaks = 1:x.num.breaks, labels = x.labels) + scale_fill_viridis(discrete = T, direction = -1)) %>% 
