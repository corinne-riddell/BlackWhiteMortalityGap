...
cod_change_results$Cause.of.death <- factor(cod_change_results$Cause.of.death, levels(cod_change_results$Cause.of.death)[c(3, 2, 4, 5, 6, 1)])
```
...
bw_dat <- BlackWhite_results %>% 
  filter(year %in% c(year1, year2, year3, year4), is.na(LE_wbgap_med) == F) %>%
  select(state, year, LE_wbgap_med, sex) %>%
  arrange(year)

bw_dat <- tidyr::spread(bw_dat, year, LE_wbgap_med) 
