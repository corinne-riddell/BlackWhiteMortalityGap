...
BlackWhite_results$Census_Region[BlackWhite_results$state %in% c("Illinois", "Indiana", "Michigan", "Ohio", 
...
BlackWhite_results$Census_Region[BlackWhite_results$state %in% c("Alabama", "Kentucky", "Mississippi", "Tennessee")] <- "East South Central"

BlackWhite_results$Census_Region[BlackWhite_results$state %in% c("Arkansas", "Louisiana", "Oklahoma", "Texas")] <- "West South Central"       

BlackWhite_results$Census_Region[BlackWhite_results$state %in% c("Arizona", "Colorado", "Idaho", "Montana", 
                                                                 "Nevada", "New Mexico", "Utah", "Wyoming")] <- "Mountain"

BlackWhite_results$Census_Region[BlackWhite_results$state %in% c("Alaska", "California", "Hawaii", "Oregon","Washington")] <- "Pacific"
...
*Changes to the differency in life expectancy in US males*
...
What we see so far (based on these 22 states):
...
- For most states, the gap decreases between 1969 and 1983, with the largest decrease found in `r diff[dim(diff)[1], ]$state` (change in gap of `r round(diff[dim(diff)[1], ]$era1.diff, 1)` (all other causes), 
`r diff[dim(diff)[1]-1, ]$state` (largely due to injuries), `r diff[dim(diff)[1]-2, ]$state` (injuries), `r diff[dim(diff)[1]-3, ]$state` (injuries), and `r diff[dim(diff)[1]-4, ]$state` (injuries).

- The gap increases for some states. Most notably in DC (CVD and other causes), Wisconsin (CVD), California (CVD). Also, for these states injuries do not contribute as much to narrowing the gap, so their gap increasing appears to be due to a combination of changes to CVD making blacks relatively worse off and less changes to injuries than experienced by other states.
...
- Generally, all other causes narrows, again much smaller contribution than injuries.

- Generally, CVD increase gap.

- Cancer increases the gap everywhere. Size of contribution to change is a little smaller than CVD.

- Generally, non-communicable increase the gap but the contribution is tiny. Notable diff is in NYC and MD it decrease gap.
...
- Except for Arizona and Kentucky, the life expectancy gap increases in each state, with the biggest driver being communicable disease (HIV/AIDS), especially in MD, NY, and CT. Injuries also contribute to the gap widening over this time frame, most notably in Wisconsin, MD, and LA. Thus, compared to previous period, these two causes of death led to a widening rather than a narrowing of the gap. 

- Similar to the firts time frame, CVD and cancer also contributed to widening the gap over time. Also similar is the minimal effect of  non-communicable disease on the gap, while all other causes contributes to narrowing the gap, with greatest contribution to narrowing in NY, CT, and FL.
...
- The gap in 1993 ranges from `r round(sum3[1, ]$LE_wbgap, 1)` [95% CI: `r round(sum3[1, ]$LE_wbgap_lcl, 1)`, `r round(sum3[1, ]$LE_wbgap_ucl, 1)`] in `r sum3[1, ]$state ` to `r round(sum3[dim(sum3)[1], ]$LE_wbgap, 1)` [95% CI: `r round(sum3[dim(sum3)[1], ]$LE_wbgap_lcl, 1)`, `r round(sum3[dim(sum3)[1], ]$LE_wbgap_ucl, 1)`] years in `r sum3[dim(sum3)[1], ]$state `.

-By 2013, it ranges from `r round(sum4[1, ]$LE_wbgap, 1)` [95% CI: `r round(sum4[1, ]$LE_wbgap_lcl, 1)`, `r round(sum4[1, ]$LE_wbgap_ucl, 1)`] in `r sum4[1, ]$state ` to `r round(sum4[dim(sum4)[1], ]$LE_wbgap, 1)` [95% CI: `r round(sum4[dim(sum4)[1], ]$LE_wbgap_lcl, 1)`, `r round(sum4[dim(sum4)[1], ]$LE_wbgap_ucl, 1)`] years in `r sum4[dim(sum4)[1], ]$state `.
...
*Changes to the differency in life expectancy in US females*

Quick summary

- Most states saw decreasing gaps in first era. Generally, all of the causes, except for cancer, were associated with changes that relatively benefitted blacks, with the biggest contributor to the narrowing gap being CVD and all other causes. However, in Wisconsin and CA, CVD led to a widening gap.

- During the second era, the gap stayed approx. the same or widened across most states. Delaware, Kentucky, Kansas, Alabama, were notable exceptions were the gap narrowed -- didn't see effect of communicable disease like other states on the gap increasing (NY, FL, DC, MD, CT, hardest hit). 

- In women, changes in injury related deaths doesn't play a big role (except DC).

- CVD differential by state. Contributes to widening the gap in CA and DC, for example, and narrowing it in KS and Kentucky.

- All other causes narrows across states (except DC, Indiana). Cancer increases the gap by a bit.

- During the third era, in-line with males, the life expectancy gap decreased during the last 20 year period, and generally speaking all the causes contributed to the gap narrowing over time. The biggest contributors were CVD and all other causes.
...
```{r figure5a, fig.align="center", fig.height=4, fig.width=11, echo=F}
...
```{r figure5b, fig.align="center", fig.height=4, fig.width=11, echo=F}
...
```{r figure5c, fig.align="center", fig.height=4, fig.width=11, echo=F}
...
```{r figure6a, fig.align="center", fig.height=5, fig.width=11, echo=F}
ggplotly(ggplot(subset(cod_decomp_results, sex == "Male" & COD == "Cancers"), aes(x = year, y = COD_cont_yrs)) + 
           ...
           geom_line(aes(col = state)) + facet_grid(.~ Census_Region) +
...
```{r, fig.align="center", fig.height=5, fig.width=11, echo=F}
...
```{r, fig.align="center", fig.height=4, fig.width=11, echo=F}
...
```{r figure6, fig.align="center", fig.height=4, fig.width=11, echo=F}
