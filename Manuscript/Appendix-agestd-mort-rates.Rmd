...
plot.mortality.trends <- function(dataset, dataset2, COD1, sex1) {
  ...
  b <- ggplot(subset.data2, aes(x = year, y = rate.difference_mean)) +
           # geom_hline(aes(yintercept = -300), lwd = 0.5, col = "grey") +
           # geom_hline(aes(yintercept = -200), lwd = 0.5, col = "grey") +
           # geom_hline(aes(yintercept = -100), lwd = 0.5, col = "grey") +
           # geom_hline(aes(yintercept = 0), lwd = 0.5, col = "grey") +
           # geom_hline(aes(yintercept = 200), lwd = 0.5, col = "grey") +
           # geom_hline(aes(yintercept = 100), lwd = 0.5, col = "grey") +
              geom_line() +
...
ggplotly(Cardiovascular.Male.mort.plot[[1]]) %>% 
  layout(xaxis = list(title = "Year (1969-2013)"), showlegend = F) 

ggplotly(Cardiovascular.Male.mort.plot[[2]]) %>% 
  layout(xaxis = list(title = "Year (1969-2013)"))

#manipulating hover info:
#p <- ggplotly(Cardiovascular.Male.mort.plot[[2]]) %>% layout(xaxis = list(title = "Year (1969-2013)"))
#p$x$data[[1]]$hoverinfo = "none" #if want no hover info for a particular trace
#p$x$data[[1]]$text #otherwise, modified what is stored here to display how you want it to look. 
```

**eFigure 2**

```{r print-em-2, fig.width = 9, fig.height = 9, echo = F}
ggplotly(Cardiovascular.Female.mort.plot[[1]]) %>% 
  layout(xaxis = list(title = "Year (1969-2013)"), showlegend = F) 

ggplotly(Cardiovascular.Female.mort.plot[[2]]) %>% 
  layout(xaxis = list(title = "Year (1969-2013)"))
```
