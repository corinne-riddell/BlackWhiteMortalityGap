...
```{r plot-arrange-as-us-map, echo=F, fig.width=18.5, fig.height=14}
...
#   for(cod in levels(mortality.rates$COD)[1]) {
    ...
    map.plot1 <- ggplot(data.sub,
      ...
      geom_ribbon(aes(ymin = rate.per.100k_lcl, ymax = rate.per.100k_ucl, group = Race), fill = "grey", col = NA) +
      geom_line(aes(lty = Race)) +
    ...
    print(map.plot1)
        ...
        map.plot2 <- ggplot(subset(mortality.rates.diff, sex == sex_i & COD == cod),
