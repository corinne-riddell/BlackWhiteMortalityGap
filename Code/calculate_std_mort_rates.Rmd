  ...
  for(i in 2:8){
    ...
    mortality.rates.summary <- mortality.rates %>% group_by(state.n, year.n, race.n, sex.n, COD) %>%
      summarise(race = first(race),
                ...
                state = first(state),
                year = first(year),
                age.std.mortality.rate_LCL = quantile(age.std.mortality.rate, 0.025), #summarise across posterior samples (post.samp)
                age.std.mortality.rate_med = quantile(age.std.mortality.rate, 0.5),
                age.std.mortality.rate_UCL = quantile(age.std.mortality.rate, 0.975))
    ...
    mortality.rates.summary <- mortality.rates.summary %>% mutate(state.reorder = reorder(state, as.numeric(Census_Division)),
                                                                  age.std.mortality.rate_LCL = age.std.mortality.rate_LCL*100000,
                                                                  age.std.mortality.rate_med = age.std.mortality.rate_med*100000,
                                                                  age.std.mortality.rate_UCL = age.std.mortality.rate_UCL*100000)
    ...
    #tidy mortality rates summary
    mortality.rates.difference$COD <- factor(mortality.rates.difference$COD, COD.levels)
    mortality.rates.difference <- add_Census_Division(mortality.rates.difference, state)
    mortality.rates.difference <- add_Census_Region(mortality.rates.difference, state)
    mortality.rates.difference$stabbrs <- state.abb[match(mortality.rates.difference$state, state.name)]
    mortality.rates.difference <- mortality.rates.difference %>% mutate(state.reorder = reorder(state, as.numeric(Census_Division)),
                                                                        rate.difference_LCL = rate.difference_LCL*100000,
                                                                        rate.difference_med = rate.difference_med*100000,
                                                                        rate.difference_UCL = rate.difference_UCL*100000)
