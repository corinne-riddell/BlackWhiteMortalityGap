...
ktm_rest_males <- read.csv("/Users/corinneriddell/Dropbox/BlackWhiteGap/males_restofstates6COD.csv")
ktm_rest_females <- read.csv("/Users/corinneriddell/Dropbox/BlackWhiteGap/females_restofstates6COD.csv")
...
for(i in 1:dim(paired.ids)[1]) {
  ...
  if(length(white.index) != 0 & length(black.index) != 0) {
    ct.white <- subset(dat.clean, StateYearRaceSex == paired.ids[i, "StateYearRaceSexWhite"]) %>% 
                ...
                rename(prop.white = cod_prop_smoothed_deaths)
    
    ct.black <- subset(dat.clean, StateYearRaceSex == paired.ids[i, "StateYearRaceSexBlack"]) %>% 
                ungroup() %>%
                select(Age2, COD2, cod_prop_smoothed_deaths) %>%
                rename(prop.black = cod_prop_smoothed_deaths)
