...
included.states <- setdiff(levels(dtotal$State2), excluded.states)
dtotal <- dtotal %>% filter(State2 %in% included.states)

save(dtotal, file = "/Users/corinneriddell/Dropbox/BlackWhiteGap/Data/SH_HA_data.Rdata")
```

```{r}
prop.table(table(is.na(dtotal$Count)))

prop.table(table(is.na(dtotal$Count[dtotal$State2 == "New York"])))
prop.table(table(is.na(dtotal$Count[dtotal$State2 == "Georgia"])))
prop.table(table(is.na(dtotal$Count[dtotal$State2 == "Florida"])))
table(is.na(dtotal$Count[dtotal$State2 == "Alabama" & dtotal$Year3 == 1990]))

test.le.fx <- function(data, state, sex, race, year) {
  test.data <- data[data["State2"] == state & data["Sex2"] == sex & data["Race2"] == race & data["Year3"] == year, ]
  lt <- life.table(data = test.data, num.ages.in.group = "nx",
             death.counts = "Count", population.counts = "Population")
