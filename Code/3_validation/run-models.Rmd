...
system.time(
  for(sex_i in c("Male", "Female")){
    for(state_i in c("Alabama", "Arizona", "Arkansas", "California")) {
      assign(paste0("r.White.", state_i, sex_i), run_analysis(dataset = dat.clean, state1 = state_i, sex1 = sex_i, race1 = "White"))
      assign(paste0("r.Black.", state_i, sex_i), run_analysis(dataset = dat.clean, state1 = state_i, sex1 = sex_i, race1 = "Black"))  
    }
  }
)

r.White <- rbind(r.White.Alabama.Male, r.White.Arizona.Male, r.White.Arkansas.Male, r.White.California.Male,
                 r.White.Alabama.Female, r.White.Arizona.Female, r.White.Arkansas.Female, r.White.California.Female)
r.White$race <- "White"

r.Black <- rbind(r.Black.Alabama.Male, r.Black.Arizona.Male, r.Black.Arkansas.Male, r.Black.California.Male,
                 r.Black.Alabama.Female, r.Black.Arizona.Female, r.Black.Arkansas.Female, r.Black.California.Female)
r.Black$race <- "Black"
```
...
```{r run_calculations}
#run time ~40 mins -- expect 5.5 hours
...
# SH_sub <- dtotal %>% filter(state == "Alabama", sex == "Male", race == "Black", year == 1990)
# 
# SH_sub %>% select(age, age_minbin, deaths, population)
```
