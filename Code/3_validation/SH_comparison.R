...
le.table <-  data.frame(state = factor(nrows), 
                        ...
                        le.white.change = numeric(nrows), le.black.change = numeric(nrows), le.gap.change = numeric(nrows),
                        le.white.change.lcl = numeric(nrows), le.black.change.lcl = numeric(nrows), le.gap.change.lcl = numeric(nrows),
                        le.white.change.ucl = numeric(nrows), le.black.change.ucl = numeric(nrows), le.gap.change.ucl = numeric(nrows))
...
system.time(
  for(state_i in setdiff(levels(dtotal$state), c("Idaho", "Montana", "North Dakota", "South Dakota", "Vermont", "Wyoming"))[1:5]) {
    ...
    le.table$le.white.change[le.table$state == state_i] <-   quantile(get(paste0("les.2009.White.", state_i)) - 
                                                                        get(paste0("les.2009.White.", state_i)), 0.5)
    le.table$le.black.change[le.table$state == state_i] <-   quantile(get(paste0("les.2009.Black.", state_i)) - 
                                                                        get(paste0("les.2009.Black.", state_i)), 0.5)
    le.table$le.gap.change[le.table$state == state_i] <-   quantile((get(paste0("les.2009.White.", state_i)) - 
    ...
    le.table$le.white.change.lcl[le.table$state == state_i] <-   quantile(get(paste0("les.2009.White.", state_i)) - 
                                                                            get(paste0("les.2009.White.", state_i)), 0.025)
    le.table$le.black.change.lcl[le.table$state == state_i] <-   quantile(get(paste0("les.2009.Black.", state_i)) - 
                                                                            get(paste0("les.2009.Black.", state_i)), 0.025)
    le.table$le.gap.change.lcl[le.table$state == state_i] <-   quantile((get(paste0("les.2009.White.", state_i)) - 
                                                                           get(paste0("les.2009.Black.", state_i))) -
                                                                          (get(paste0("les.1990.White.", state_i)) - 
                                                                             get(paste0("les.1990.Black.", state_i))), 0.025) 
    ...
    le.table$le.white.change.lcl[le.table$state == state_i] <-   quantile(get(paste0("les.2009.White.", state_i)) - 
                                                                            get(paste0("les.2009.White.", state_i)), 0.975)
    le.table$le.black.change.lcl[le.table$state == state_i] <-   quantile(get(paste0("les.2009.Black.", state_i)) - 
                                                                            get(paste0("les.2009.Black.", state_i)), 0.975)
    le.table$le.gap.change.lcl[le.table$state == state_i] <-   quantile((get(paste0("les.2009.White.", state_i)) - 
                                                                           get(paste0("les.2009.Black.", state_i))) -
                                                                          (get(paste0("les.1990.White.", state_i)) - 
                                                                             get(paste0("les.1990.Black.", state_i))), 0.975) 
