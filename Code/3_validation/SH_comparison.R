...
nrows <- 3*length(levels(dtotal$state))
...
# Males 1990 and 2009
system.time(
  for(state_i in setdiff(levels(dtotal$state), c("Idaho", "Montana", "North Dakota", "South Dakota", "Vermont", "Wyoming"))[1:5]) {
     for(race_i in c("White", "Black")) {
       ...
       life.tables.2009 <- map(r2009, ~ life.table( .x, num.ages.in.group = "age_bin_size",
                                                    death.counts = "smoothed_deaths",
                                                    population.counts = "population",
                                                    ave.prop.le1.lived = 0.10))
       ...
       assign(paste0("les.2009.", race_i, ".", state_i), map_dbl(life.tables.2009, ~ .x$e_x[1]))
    ...
    for(time_i in c("1990", "2009")) {
      ...
      le.table$le.white[le.table$state == state_i & le.table$year == time_i] <- quantile(get(paste0("les.", time_i , ".White.", state_i)), 0.5)
      le.table$le.black[le.table$state == state_i & le.table$year == time_i] <- quantile(get(paste0("les.", time_i, ".Black.", state_i)), 0.5)
      le.table$le.gap[le.table$state == state_i & le.table$year == time_i] <-   quantile(get(paste0("les.", time_i, ".White.", state_i)) - 
                                                                                           get(paste0("les.", time_i, ".Black.", state_i)), 0.5)
      
      le.table$le.white.lcl[le.table$state == state_i & le.table$year == time_i] <- quantile(get(paste0("les.", time_i , ".White.", state_i)), 0.025)
      le.table$le.black.lcl[le.table$state == state_i & le.table$year == time_i] <- quantile(get(paste0("les.", time_i, ".Black.", state_i)), 0.025)
      le.table$le.gap.lcl[le.table$state == state_i & le.table$year == time_i] <-   quantile(get(paste0("les.", time_i, ".White.", state_i)) - 
                                                                                           get(paste0("les.", time_i, ".Black.", state_i)), 0.025) 
      
      le.table$le.white.ucl[le.table$state == state_i & le.table$year == time_i] <- quantile(get(paste0("les.", time_i , ".White.", state_i)), 0.975)
      le.table$le.black.ucl[le.table$state == state_i & le.table$year == time_i] <- quantile(get(paste0("les.", time_i, ".Black.", state_i)), 0.975)
      le.table$le.gap.ucl[le.table$state == state_i & le.table$year == time_i] <-   quantile(get(paste0("les.", time_i, ".White.", state_i)) - 
                                                                                           get(paste0("les.", time_i, ".Black.", state_i)), 0.975) 
      
    }
    
    le.table$le.white[le.table$state == state_i & le.table$year == "difference"] <-   quantile(get(paste0("les.2009.White.", state_i)) - 
                                                                                                 get(paste0("les.1990.White.", state_i)), 0.5)
    le.table$le.black[le.table$state == state_i & le.table$year == "difference"] <-   quantile(get(paste0("les.2009.Black.", state_i)) - 
                                                                                                 get(paste0("les.1990.Black.", state_i)), 0.5)
    
    le.table$le.gap[le.table$state == state_i & le.table$year == "difference"] <-   quantile((get(paste0("les.2009.White.", state_i)) - 
                                                                                                get(paste0("les.2009.Black.", state_i))) -
                                                                                               (get(paste0("les.1990.White.", state_i)) - 
                                                                                                  get(paste0("les.1990.Black.", state_i))), 0.5) 

    le.table$le.white.lcl[le.table$state == state_i & le.table$year == "difference"] <-   quantile(get(paste0("les.2009.White.", state_i)) - 
                                                                                                 get(paste0("les.1990.White.", state_i)), 0.025)
    le.table$le.black.lcl[le.table$state == state_i & le.table$year == "difference"] <-   quantile(get(paste0("les.2009.Black.", state_i)) - 
                                                                                                 get(paste0("les.1990.Black.", state_i)), 0.025)
    
    le.table$le.gap.lcl[le.table$state == state_i & le.table$year == "difference"] <-   quantile((get(paste0("les.2009.White.", state_i)) - 
                                                                                                get(paste0("les.2009.Black.", state_i))) -
                                                                                               (get(paste0("les.1990.White.", state_i)) - 
                                                                                                  get(paste0("les.1990.Black.", state_i))), 0.025) 
    
    le.table$le.white.ucl[le.table$state == state_i & le.table$year == "difference"] <-   quantile(get(paste0("les.2009.White.", state_i)) - 
                                                                                                 get(paste0("les.1990.White.", state_i)), 0.975)
    le.table$le.black.ucl[le.table$state == state_i & le.table$year == "difference"] <-   quantile(get(paste0("les.2009.Black.", state_i)) - 
                                                                                                 get(paste0("les.1990.Black.", state_i)), 0.975)
    
    le.table$le.gap.ucl[le.table$state == state_i & le.table$year == "difference"] <-   quantile((get(paste0("les.2009.White.", state_i)) - 
                                                                                                get(paste0("les.2009.Black.", state_i))) -
                                                                                               (get(paste0("les.1990.White.", state_i)) - 
                                                                                                  get(paste0("les.1990.Black.", state_i))), 0.975) 
