...
le5 <- rep(NA, length(non.zero.stratums))
le9 <- rep(NA, length(non.zero.stratums))
...
les <- data.frame(life.expectancy.birth = le, life.expectancy.birth.5 = le5, life.expectancy.birth.9 = le9, StateYearRaceSex = non.zero.stratums)
...
Black <- dat2 %>% filter(Race2 == "Black" & Age3 == 0) %>% 
                  select(State2, Year3, Sex2, Pop_across_age, life.expectancy.birth, life.expectancy.birth5, life.expectancy.birth9) %>%
                  rename(Pop_black = Pop_across_age,  
                         le.birth.black = life.expectancy.birth, 
                         le.birth.black5 = life.expectancy.birth5, 
                         le.birth.black9 = life.expectancy.birth9)

White <- dat2 %>% filter(Race2 == "White" & Age3 == 0) %>% 
                  select(State2, Year3, Sex2, Pop_across_age, life.expectancy.birth, life.expectancy.birth5, life.expectancy.birth9) %>%
                  rename(Pop_white = Pop_across_age,  
                         le.birth.white = life.expectancy.birth,
                         le.birth.white5 = life.expectancy.birth5,
                         le.birth.white9 = life.expectancy.birth9)
...
for(i in 1:dim(paired.ids)[1]) {
    if(list.cod.decomp.tables[[i]][1] != "Couldn't compare life tables") { 
        list.cod.decomp.tables[[i]] <- 
                              make_dataset_cod_plot(cod.decomp.table = list.cod.decomp.tables[[i]], 
                              age.groups = "Ages", 
