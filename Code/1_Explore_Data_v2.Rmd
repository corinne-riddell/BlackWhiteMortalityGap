...
#create list of stratum.ids for each race-sex combination
for (rs in unique(levels(dat.clean$RaceSex))) {
  assign(paste0(gsub(" ", "", rs), ".stratum.ids"), 
      dat.clean %>% filter(dat.clean$RaceSex == rs) %>%
        select(StateYearRaceSex, StateYearSex) %>%
        group_by(StateYearRaceSex, StateYearSex) %>%
        filter(row_number() == 1)# %>%
      #  rename(paste0(gsub(" ", "", rs), ".ids") = StateYearRaceSex)
         )
  
}

#merge the black-white lists
Female.ids <- merge(WhiteFemale.stratum.ids, BlackFemale.stratum.ids, by = "StateYearSex")
Female.ids <- Female.ids %>% rename(White.StateYearRaceSex = StateYearRaceSex.x, Black.StateYearRaceSex = StateYearRaceSex.y)
Female.ids$Sex2 <- "Female"

Male.ids <- merge(WhiteMale.stratum.ids, BlackMale.stratum.ids, by = "StateYearSex")
Male.ids <- Male.ids %>% rename(White.StateYearRaceSex = StateYearRaceSex.x, Black.StateYearRaceSex = StateYearRaceSex.y)
Male.ids$Sex2 <- "Male"
...
for(i in 1:dim(paired.ids)[1]) {
  white.index <- which(non.zero.stratums == paired.ids[i, 2])
  black.index <- which(non.zero.stratums == paired.ids[i, 3])
...
```{r}
save(dat.clean, dat2, BlackWhite, file = "/Users/corinneriddell/Dropbox/BlackWhiteGap/Data/main_datasets.Rdata")
```
