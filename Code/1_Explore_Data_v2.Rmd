...
#still need to convince myself this works
...
for(i in 1:dim(paired.ids)[1]) {
  ...
  if(length(white.index) != 0 & length(black.index) != 0) {
    ct.white <- subset(dat.clean, StateYearRaceSex == paired.ids[i, "StateYearRaceSexWhite"]) %>% 
                ungroup() %>%
                select(Age2, Age3, COD2, cod_prop_death5) %>%
    ...
    list.cod.decomp.tables[[i]] <- cause_of_death_decomp(life.table1 = as.data.frame(list.life.tables[[black.index]]), 
                                                 ...
                                                 cod.table = ct.both, "Age2", "COD2", "prop1", "prop2")
  } else {
...
test1 <- cause_of_death_decomp(life.table1 = as.data.frame(list.life.tables[[black.index]]),
                               life.table2 = as.data.frame(list.life.tables[[white.index]]), 
                               decomp.table = as.data.frame(list.age.decomp.tables[[i]]),
                               cod.table = ct.both, 
...
test2 <- cause_of_death_decomp(life.table2 = as.data.frame(list.life.tables[[black.index]]), 
                                                 life.table1 = as.data.frame(list.life.tables[[white.index]]), 
                                                 decomp.table = as.data.frame(list.age.decomp.tables[[i]]),
                                                 cod.table = ct.both, "Age2", "COD2", "prop1", "prop2")

age_contr1 <- test1 %>% ungroup() %>% group_by(Ages) %>% summarise(age_contribution = sum(C_xi))
age_contr1
list.age.decomp.tables[[i]]
#compare -- see that there is something wrong with the age labels in one or the other... 
#all the same numbers but some of them are in a strange order.

age_contr2 <- test2 %>% ungroup() %>% group_by(Ages) %>% summarise(age_contribution = sum(C_xi))
sum(age_contr2$age_contribution)
```
