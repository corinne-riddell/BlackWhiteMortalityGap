...
mortality.rates.wide <- read.csv("/Users/corinneriddell/Documents/repos/BlackWhiteMortalityGap/Data/mortality_rates_diff_1.csv")
```
...
for(sex_i in c("Male", "Female")) {
  for(cod in levels(mortality.rates$COD)) {
    ...
    a <- ggplot(data.sub, 
                aes(y = age.std.mortality.rate_med*100000, x = year)) + 
      geom_vline(aes(xintercept = 1983), col = "#636363", lty = 3) +
      geom_vline(aes(xintercept = 1993), col = "#636363", lty = 3) +
      geom_ribbon(aes(ymin = age.std.mortality.rate_LCL*100000, ymax = age.std.mortality.rate_UCL*100000, col = race)) +
