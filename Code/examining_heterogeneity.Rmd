...
cod_decomp_results$COD_cont_yrs_SE1 <- (cod_decomp_results$COD_cont_yrs_ucl - cod_decomp_results$COD_cont_yrs_mean)/1.96
cod_decomp_results$COD_cont_yrs_SE2 <- -(cod_decomp_results$COD_cont_yrs_lcl - cod_decomp_results$COD_cont_yrs_mean)/1.96
cod_decomp_results$SE.diff <- cod_decomp_results$COD_cont_yrs_SE1 - cod_decomp_results$COD_cont_yrs_SE2
summary(cod_decomp_results$SE.diff)

# cod_decomp_results.means <- cod_decomp_results %>% 
#   mutate(COD_cont_yrs_width = COD_cont_yrs_ucl - COD_cont_yrs_lcl) %>%
#   group_by(state, sex, COD) %>%
#   summarise(mean_COD_cont_yrs_width = mean(COD_cont_yrs_width))
# 
# mod1 <- lm(COD_cont_yrs_mean ~ bs(year, degree = 1, df = 5) + state, data = subset.data)
# 
# mod2 <- lm(COD_cont_yrs_mean ~ bs(year, degree = 1, df = 5) + state, weights = 1/(COD_cont_yrs_SE1)^2, 
#            data = subset.data)
```
...
plot.resid.trends <- function(dataset, COD1, sex1) {
  ...
  a <- ggplot(subset.data,
    ...
    geom_ribbon(aes(ymin = COD_cont_yrs_lcl, ymax = COD_cont_yrs_ucl), fill = "#b2e2e2") +
    #scale_fill_manual(values = c("#b2e2e2")) +
    facet_wrap( ~ order) + 
    geom_line(aes(y = .fitted), col = "#006d2c") +
