...
RE.models.infants <- age_cod_results_infants_AOC %>% 
  group_by(sex) %>%
  do(fit = lmer(age_COD_cont_yrs_mean ~ bs(year, degree = 1, df = 5) + (1|state),
...
RE.estimates.infants <- c(RE.models.infants$RE.estimates[[1]]$state$`(Intercept)`, RE.models.infants$RE.estimates[[2]]$state$`(Intercept)`)
...
write.csv(age_cod_results_infants_AOC, "/Users/corinneriddell/Documents/repos/BlackWhiteMortalityGap/Results2/age_cod_results_infants_AOC.csv")
```
